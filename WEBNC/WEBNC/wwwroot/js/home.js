fetch('https://localhost:7047/api/customer/products')
    ...
    .then(sanpham => {
        ...
        if (listStartElement) {
            ...
            // 2.Reset lại Carousel trước khi chạy
            var $carousel = $("#listStart");

            // Hủy instance cũ nếu có (để tránh xung đột với home.js)
            $carousel.trigger('destroy.owl.carousel');

            // Xóa sạch các class trạng thái cũ mà thư viện để lại
            $carousel.removeClass("owl-loaded owl-drag owl-hidden");

            // Tìm và xóa thẻ owl-stage-outer nếu còn sót lại (dọn dẹp sạch sẽ)
            $carousel.find('.owl-stage-outer').children().unwrap();

            // 3. Khởi tạo lại từ đầu
